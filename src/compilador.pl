...
keywords([definir,como,hacer,repetir,mientras,entonces,veces,si,sino]).

token(keyword(Token),[]) --> palabra(ListaChar), {atom_chars(Token,ListaChar), keywords(KEYWORDS),memberchk(Token,KEYWORDS)}.
token(nombre(Token),[]) --> palabra(ListaChar), {atom_chars(Token,ListaChar)}.
...
palabra([Letra|Resto]) --> letra(Letra), restoPalabra(Resto).

restoPalabra([Letra|Resto]) --> letra(Letra), !, restoPalabra(Resto).
restoPalabra([Digito|Resto]) --> digito(Digito), !, restoPalabra(Resto).
restoPalabra([Menos|Resto]) --> [Menos], {Menos='-'}, !, restoPalabra(Resto).

restoPalabra([Letra]) --> letra(Letra).
restoPalabra([Digito]) --> digito(Digito).
restoPalabra([]) --> [].
